...
public class EventServerMain {
	...
	private EventManagerMain manager = null;
	 ...
	 */
	public EventServerMain(String[] args) {
		if (args.length > 0)
    		parseArgs(args);
    	try {
    		if (hostName.length() == 0)
    			hostName = InetAddress.getLocalHost().getHostAddress();
			manager = new EventManagerMain(true);
			...
			for(String port : clusterArr) {
				int p = Integer.valueOf(port);
				managerMap.put(p, 0);
			}
			manager.setManagerMap(managerMap);
			manager.startService(hostName);
		} catch (Exception e) {
     ...
     */
	public EventServerMain(String[] args, EventManager manager) {
		if (args.length > 0)  
    		parseArgs(args);
    	try {
    		if (hostName.length() == 0) 
	...
	private void parseArgs(String args[]) {
		...
		for (int i = 0; i < args.length; i ++) {	
			...
			else if (args[i].equals("-cluster")) cluster = args[++i];
			else {
	...
	public static void main(String[] args) throws RemoteException {
		EventServerMain server = new EventServerMain(args);
